...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            prenodes_prev_intersection = a_node.prenodes.intersection(self.s_levels[level])
            if len(prenodes_prev_intersection) > 0:
                for parent in prenodes_prev_intersection:
                    parent.children.add(a_node)
                    a_node.parents.add(parent)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_eff in action_a1.effect_add:
            if pos_eff in action_a2.effect_rem:
                return True
        for pos_eff in action_a2.effect_add:
            if pos_eff in action_a1.effect_rem:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        action_a1 = node_a1.action
        action_a2 = node_a2.action
        for neg_eff in action_a1.effect_rem:
            if neg_eff in action_a2.precond_pos:
                return True
        for neg_eff in action_a2.effect_rem:
            if neg_eff in action_a1.precond_pos:
                return True
        for pos_eff in action_a1.effect_add:
            if pos_eff in action_a2.precond_neg:
                return True
        for pos_eff in action_a2.effect_add:
            if pos_eff in action_a2.precond_neg:
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        for prenode1 in node_s1.parents:
            for prenode2 in node_s2.parents:
                if not prenode1.is_mutex(prenode2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        if not isinstance(goals, list):
            goals = [goals]
        goal_nodes = [PgNode_s(goal, True) for goal in goals]
